...
export function apply(ctx) {
  ...
  function fmtTime(ts) { try { return new Date(ts).toLocaleTimeString('zh-CN', { hour12: false }) } catch (e) { return '' } }
  ...
  function Dashboard() {
    ...
    return h('div', { className: 'st-wrap' },
      h('div', { className: 'st-row' },
        ...
        h(Toggle, { k: 'compactAssist', label: 'Compact@' + fmtTok(cm.budgetTok || 120000), on: (cm.budgetTok || 0) > 0, onChange: refresh }),
        h('button', {
          className: 'st-btn',
      ...
      h('div', { className: 'st-card' },
        ...
        (d.byTool || []).length === 0 ? h('div', { className: 'st-dim', style: { marginTop: 8 } }, 'nothing compressed yet') :
