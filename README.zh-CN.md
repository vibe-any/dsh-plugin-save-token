...
| 场景 | 输入 | 结果 | 说明 |
|---|---|---|---|
| CLI 价格表（400 行管道表格） | 41,727 B | **15,191 B（−64%）** | 头尾全行 + 中段每 7 行带行号采样，全表结构可见 |
| 模型价格 JSON 注册表（300 条均匀数组） | 34,000 B | **19,935 B（−41%）** | TOON 无损路线，零信息损失 |
| 真实会话累计（14 个大输出事件，146,935 B） | — | 最优单事件 **−96%**（33,184→1,317 B）；最差 −31% | 有损路径兜底，最差情况也过双门控 |
| 反面教材 → 设计动因 | 35.5KB LiteLLM 价格注册表曾被盲目头尾窗口化，子代理找不到中间行反复重查 | v2 改为结构感知策略 | 诚实记录：这就是优化 1 的由来 |

> 以上为开发环境实测数字，非实验室基准；你的收益取决于工作负载中大输出的占比。
...
装好后无需任何操作：打开 **Settings → Token Saver** 看完整面板，输入框下方有常驻实时条。三个开关（Compress / Dedupe / Compact@120k）可在面板上一键切换。
