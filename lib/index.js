...
function apply(ctx, config) {
  ...
  function transformValue(v, depth) {
    ...
    if (Array.isArray(v)) {
      ...
      return v.map(function(x) {
        return transformValue(x, depth + 1);
      });
    }
  ...
  function compressJson(text) {
    ...
    if (dom !== null) {
      ...
      if (ukeys !== null) {
        ...
        if (Array.isArray(parsed)) {
          ...
          var rest = {};
          for (var rk in parsed) {
            if (parsed[rk] !== dom.arr) rest[rk] = parsed[rk];
          }
    ...
    var rebuilt;
    try {
      rebuilt = JSON.stringify(transformValue(parsed, 0));
    } catch (e) {
      return null;
    }
  ...
  function buildCandidate(text) {
    ...
    return { text: candidate, before, after, lossless: lastLossless === true };
  }
  ...
  ctx.on("llm/stream", function(options, next) {
    ...
    function closeRecord() {
      ...
      if (fullSid !== void 0) lastEstBySession.set(fullSid, rec.estPrompt);
      ...
      totals.avoidedTokens += rec.avoided;
      ...
      noteRecent(
        ...
        "prompt~" + fmtInt(rec.estPrompt) + " tok, out " + fmtInt(rec.output) + ", avoided ~" + fmtInt(rec.avoided) + (rec.replayHits ? " (" + rec.replayHits + " replayed)" : ""),
        rec.avoided
      );
    }
    var inner = next();
  ...
  ctx.on("agent/pre-step", async function(payload, next) {
    try {
      ...
      if (sid !== void 0 && cfg.compactBudgetTokens > 0) {
        ...
        if (est > cfg.compactBudgetTokens && now - lastAt > cfg.compactCooldownMs) {
          ...
          if (cs !== void 0 && typeof cs.compactIfNeeded === "function") {
            ...
            if (res !== void 0 && res !== null) {
              compactStats.done++;
              noteRecent("compact", payload.agent.options && payload.agent.options.model ? payload.agent.options.model : "session", "context ~" + fmtInt(est) + " tok > budget -> compaction applied", 0);
            } else {
              compactStats.skipped++;
              noteRecent("compact", "policy", "pressure reported at ~" + fmtInt(est) + " tok; engine declined (no-op)", 0);
            }
          }
        }
      }
  ...
  function setEnabled(args) {
    ...
    else if (a.key === "compactAssist") cfg.compactBudgetTokens = a.value ? 12e4 : 0;
